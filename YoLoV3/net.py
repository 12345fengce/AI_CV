...
class YoLoNet(nn.Module):
    def __init__(self):
        ...
        self.out3 = nn.Sequential(ConvSet(512, 256),
                                            ...
                                            Convolutional(512, 21, 1, 1))  # 输出通道3

    def forward(self, x):
        dn_1 = self.darknet1(x)
        dn_2 = self.darknet2(dn_1)
        dn_3 = self.darknet3(dn_2)

        s_1 = self.set1(dn_3)
        o_1 = self.out1(s_1)

        up_1 = self.up1(s_1)
        c_1 = torch.cat((up_1, dn_2), dim=1)

        s_2 = self.set2(c_1)
        o_2 = self.out2(s_2)

        up_2 = self.up2(s_2)
        c_2 = torch.cat((up_2, dn_1), dim=1)
